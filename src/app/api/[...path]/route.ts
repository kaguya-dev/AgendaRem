import { z } from 'zod';
import { authenticated, cookie, cronAuth, sameOrigin, sessionToken, verifyPassword } from '@/lib/auth';
import { db, consumeLimit } from '@/lib/db';
import { DomainError } from '@/lib/domain';
import { chat, cleanup, panelAction, snapshot } from '@/lib/service';
import { deleteProvider, listProviders, resetProvider, saveProvider } from '@/lib/llm';
...
async function handler(request: Request, context: { params: Promise<{ path: string[] }> }) {
  try {
    ...
    if (path === 'auth' && method === 'GET') return reply({ authenticated: authenticated(request) });
    ...
    if (path === 'login' && method === 'POST') {
      sameOrigin(request);
      const body = z.object({ password: z.string().min(1).max(256) }).parse(JSON.parse(await read(request)));
    ...
    if (path === 'waha' || path.startsWith('internal/') || path === 'simulate' || path === 'retry')
    ...
    if (path === 'chat') {
      const value = z.object({ text: z.string().trim().min(1).max(6000), requestId: z.string().uuid() }).parse(body);
    ...
    if (error instanceof z.ZodError)
      return reply({ error: 'Dados inválidos. Confira os campos.', details: error.issues.map((i) => ({ field: i.path.join('.'), message: i.message })) }, 400);
    ...
    return reply({ error: 'Não foi possível concluir a operação. Verifique a configuração e a conexão com o banco.' }, 500);
  }
}
